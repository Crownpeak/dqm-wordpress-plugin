(function addDqmCmsButton() {
        ...
        if (!panel && show) {
            ...
            function renderScoreCard(passedCount, totalCount) {
                ...
                let html = `
                    ...
                    <div class="card">
                        <div class="highlight-button-container" style="text-align: right; margin-bottom: 10px;">
                            <button id="dqm-highlight-button" class="button button-primary">Highlight Issues</button>
                        </div>
                ...
                const highlightButton = scoreCardContainer.querySelector('#dqm-highlight-button');
                if (highlightButton) {
                    highlightButton.addEventListener('click', function() {
                        if (this.classList.contains('active')) {
                            removeHighlighting();
                            this.classList.remove('active');
                            this.textContent = 'Highlight Issues';
                        } else {
                            highlightIssuesInEditor(lastAssetId);
                        }
                    });
                }
    ...
    function highlightIssuesInEditor(assetId) {
        ...
        if (!assetId) {
            console.error('No asset ID available. Please run a quality check first.');
            alert('No asset ID available. Please run a quality check first.');
            return;
        }
        ...
        if (!apiKey) {
            ...
            alert('API key not found. Please check your credentials.');
            return;
        }
        
        const highlightButton = document.querySelector('#dqm-highlight-button');
        const isHighlightActive = highlightButton && highlightButton.classList.contains('active');
        ...
        loadingDiv.id = 'dqm-highlight-loading';
        loadingDiv.innerHTML = '<span>Loading highlights...</span>';
        ...
        const url = `https://api.crownpeak.net/dqm-cms/v1/assets/${assetId}/pagehighlight/all?apiKey=${apiKey}&visibility=public`;
        ...
        .then(responseText => {
            ...
            if (responseText.trim().startsWith('{') && responseText.trim().endsWith('}')) {
                try {
                    ...
                    console.warn('Response looks like JSON but couldn\'t be parsed:', jsonErr);
                }
            }
        ...
        .then(htmlContent => {
            ...
            try {
                htmlContent = htmlContent.replace(/(<svg[^>]*?)([a-zA-Z0-9-]+)=\\\"([^\\]+)\\\"/g, '$1$2="$3"');
                htmlContent = htmlContent.replace(/(viewBox|d|points|transform)=\\\"([^\\]+)\\\"/g, '$1="$2"');
                htmlContent = htmlContent.replace(/(<path[^>]*?)([a-zA-Z0-9-]+)=\\\"([^\\]+)\\\"/g, '$1$2="$3"');
                htmlContent = htmlContent.replace(/(<rect[^>]*?)([a-zA-Z0-9-]+)=\\\"([^\\]+)\\\"/g, '$1$2="$3"');
                htmlContent = htmlContent.replace(/(<circle[^>]*?)([a-zA-Z0-9-]+)=\\\"([^\\]+)\\\"/g, '$1$2="$3"');
                htmlContent = htmlContent.replace(/(<polygon[^>]*?)([a-zA-Z0-9-]+)=\\\"([^\\]+)\\\"/g, '$1$2="$3"');
            } catch (err) {
                console.warn('Error pre-processing HTML content:', err);
            }
            
            const tempDiv = document.createElement('div');
            tempDiv.style.display = 'none';
            
            const fixedHtmlContent = htmlContent.replace(/([a-zA-Z0-9-]+)=\\\"([^\\]+)\\\"/g, '$1="$2"');
            
            try {
                tempDiv.innerHTML = fixedHtmlContent;
            } catch (err) {
                console.error('Error parsing fixed HTML content:', err);
                
                try {
                    const noSvgContent = fixedHtmlContent.replace(/<svg[^>]*>.*?<\/svg>/gs, '');
                    tempDiv.innerHTML = noSvgContent;
                } catch (fallbackErr) {
                    console.error('Fallback error handling failed:', fallbackErr);
                    tempDiv.innerHTML = '<div class="dqm-highlight-error">Error displaying highlights</div>';
                }
            }
            ...
            try {
                const highlightedElements = tempDiv.querySelectorAll('.dqm-highlight, .dqm-issue');
                
                if (htmlContent.indexOf('dqm-highlight') === -1 && htmlContent.indexOf('dqm-issue') === -1) {
                    console.log('No highlight components found in the API response');
                    alert('No highlight components found in the content. Nothing to highlight.');
                        ...
                        try {
                            ...
                            svgElements.forEach(svg => {
                                if (svg.getAttribute('height') && svg.getAttribute('height').includes('\\\"')) {
                                    const fixedHeight = svg.getAttribute('height').replace(/\\"/g, '');
                                    svg.setAttribute('height', fixedHeight);
                                }
                                if (svg.getAttribute('width') && svg.getAttribute('width').includes('\\\"')) {
                                    const fixedWidth = svg.getAttribute('width').replace(/\\"/g, '');
                            ...
                            if (elementText) {
                                const blockContent = jQuery('div.block-editor-block-list__layout').find(':contains("' + elementText + '")');
                                if (blockContent.length > 0) {
                                    blockContent.addClass('dqm-element-highlight');
                                }
                            }
                        } catch (err) {
                            console.warn('Error processing highlight element:', err);
                        }
                    });
                }
            } catch (err) {
                console.error('Error processing highlighted elements:', err);
            }
            
            if (!foundElements) {
                console.log('No specific elements found to highlight, applying fallback highlighting');
                if (htmlContent.indexOf('dqm-highlight') !== -1 || htmlContent.indexOf('dqm-issue') !== -1) {
                    ...
                    console.log('No highlight components found in the response, skipping fallback highlighting');
        ...
        .catch(error => {
            ...
            if (error.message && error.message.includes('No highlight components')) {
                console.log('No highlight components available, skipping fallback highlighting');
                ...
                console.log('Error fetching highlights, applying fallback highlighting');
                addHighlightStyles();
                applyFallbackHighlighting();
                ...
                alert('Error fetching specific highlights. Applied general highlighting instead.');
            }
        });
    }
    ...
    function addHighlightStyles() {
        if (!document.getElementById('dqm-highlight-styles')) {
            ...
            style.textContent = `
                .dqm-element-highlight {
                    outline: 2px dashed #b604d4 !important;
                    outline-offset: 2px !important;
                }
                .dqm-element-highlight:hover {
                    outline: 3px solid #b604d4 !important;
                }
            `;
    ...
    function applyFallbackHighlighting() {
        const editorContent = document.querySelector('.editor-styles-wrapper, .block-editor-block-list__layout');
        if (editorContent) {
            const contentElements = editorContent.querySelectorAll('h1, h2, h3, h4, h5, h6, p, img, figure, table, ul, ol, blockquote');
            contentElements.forEach(element => {
                element.classList.add('dqm-element-highlight');
            });
        }
    }
    
    function removeHighlighting() {
        const highlightedElements = document.querySelectorAll('.dqm-element-highlight');
        highlightedElements.forEach(element => {
            element.classList.remove('dqm-element-highlight');
        });
        
        const highlightStyles = document.getElementById('dqm-highlight-styles');
        if (highlightStyles) {
